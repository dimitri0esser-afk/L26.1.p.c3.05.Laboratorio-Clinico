...
export default class Cl_vLaboratorio implements I_vLaboratorio {
  ...
  private onSeleccionarEstudioCallback: ((nombreEstudio: string) => void) | null = null;
  ...
  limpiarFiltrosBusqueda(): void {
    ...
    if (this.lblResultadosBusqueda) this.lblResultadosBusqueda.textContent = "";
  }

  private actualizarContadorResultados(cantidad: number, total: number): void {
    if (!this.lblResultadosBusqueda) return;
    if (cantidad === total) {
 ...
 mostrarModalEstudios(examenes: any[]): void {
  ...
  mostrarEstadisticas(estadisticas: {
    ...
    const selEstudio = document.getElementById("selEstudioPromedio") as HTMLSelectElement;
  ...
  limpiarSeleccion(): void {
    if (this.divEstudios) this.divEstudios.innerHTML = "";
    if (this.divCobranza) this.divCobranza.innerHTML = "";
  }
}